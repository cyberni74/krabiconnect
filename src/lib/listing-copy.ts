...
export function listingTitleTh(row: ListingCopySource): string {
  return firstNonEmpty(row.titleTh, row.title_th, row.listingTitleTh, row.listing_title_th);
}

export function listingTitleEn(row: ListingCopySource): string {
  return firstNonEmpty(row.titleEn, row.title_en, row.listingTitleEn, row.listing_title_en);
}
